...
| 1 | Arabic as the default website language | **Deferred to Phase 5**, and done there. `ar_001` is installed and active, Arabic is the website default and English moved to `/en`. It was deferred rather than taken in Phase 1 to avoid shipping an Arabic site whose content was untranslated English; the RTL discipline in CLAUDE.md applied to every stylesheet written in the meantime, which is why the switch needed no retrofit |
...
| 5 | Arabic digits: Eastern (٠١٢) vs Western (012) | **Western digits** everywhere, in both languages. Prices, age ranges and phone numbers already render Western in Odoo's `ar_001` locale, and Lebanese storefronts read that way; mixing the two within one page is the failure mode worth avoiding |
| 6 | Card processing | **Cash on delivery.** No card acquirer for launch. Stripe does not onboard Lebanese entities and a regional acquirer means a custom payment provider module; neither is worth blocking launch on. Revisit when volume justifies it. Delivery pricing and the free-shipping threshold are still open |
